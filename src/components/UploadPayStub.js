...
const PayStubFile = (props) => {
  const [count, setCount] = useState(0);
  const { show, payStub, setPayStubCheck, animalFile, setState } = props;

  useEffect(() => {
    const intervalId = setInterval(() => {
      if (count === 100) {
        clearInterval(intervalId);
        setPayStubCheck(false);
        setState(show+1)
      } else {
        setCount(count + 100);
      }
